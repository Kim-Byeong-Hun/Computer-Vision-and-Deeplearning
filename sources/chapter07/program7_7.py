# 우편번호 인식기 v.1(DMLP버전) 구현하기
import os
os.chdir('C:/Users/qudgn/Computer-Vision-and-Deeplearning/sources/chapter07')
...
def recongition():
    numerials = grab_numerals()
    numerials = numerials.reshape(5, 784)
    numerials = numerials.astype(np.float32) / 255.0
    res = model.predict(numerials)
...
def writing(event, x, y, flags, param):
    if event == cv.EVENT_LBUTTONDOWN:
        cv.circle(img, (x,y), Brushiz, LColor, -1)
